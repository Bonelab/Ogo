...
def ImageChangeExtent(input_file, output_file, offset, extent, filler, overwrite, quiet):
    ...
    if offset is not None:
        ...
        # Validate negative offsets don't exceed half dimension
        if ox < 0 and abs(ox) > sx // 2:
            ogo.message(f'[ERROR] Negative X offset ({ox}) exceeds half the X dimension ({sx // 2})')
            return 1
        if oy < 0 and abs(oy) > sy // 2:
            ogo.message(f'[ERROR] Negative Y offset ({oy}) exceeds half the Y dimension ({sy // 2})')
            return 1
        if oz < 0 and abs(oz) > sz // 2:
            ogo.message(f'[ERROR] Negative Z offset ({oz}) exceeds half the Z dimension ({sz // 2})')
            return 1
        ...
        # Extent already validated by validate_extent in main()
...
OFFSET MODE (--offset X Y Z):
  ...
  - Negative offsets cannot exceed half the dimension size
...
'''
    ...
    # Validate extent if provided
    if args.extent is not None:
        try:
            validate_extent(args.extent)
