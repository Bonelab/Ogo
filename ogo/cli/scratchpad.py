...
def internal(input_image, input_mask, output_image, MonteCarlo, calib_file_name, useLabels, useL4, overwrite, func):
    ...
    if MonteCarlo:
        ...
        for k in range(npBMD_iterations.shape[1]):
            mean_BMD = np.mean(npBMD_iterations[:,k])
            means.append(mean_BMD)
            std_dev_BMD = np.std(npBMD_iterations[:,k])
            stds.append(std_dev_BMD)
	   
        normality_BMD = ogo.determine_normality(BMD_iterations)
        if normality_BMD == True:
            pass
        elif normality_BMD == False:
            ogo.message('WARNING: Average BMD is not normally distributed. ')
            ogo.message('Range may not be completely accurate.')
        
        dict_BMD = {'mean':means, 'std': stds}
        df = pd.DataFrame(dict_BMD)
        df.to_excel('/Users/brynmatheson/Desktop/CTDXA0053/BMDAVG_mean_std.xlsx', index=True)


        df2 = pd.DataFrame(BMD_iterations)
        df2.to_excel('/Users/brynmatheson/Desktop/CTDXA0053/BMDAVG.xlsx', index=True)

        dict_parameters = {'hu_to_mass_attenuation_slopes': hu_to_mass_attenuation_slopes, 'hu_to_mass_attenuation_intercepts': hu_to_mass_attenuation_intercepts, 
        ...
        df3 = pd.DataFrame(dict_parameters)
        df3.to_excel('/Users/brynmatheson/Desktop/CTDXA0053/CalibrationParameters.xlsx', index=True)
