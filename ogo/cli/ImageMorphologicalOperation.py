...
# +------------------------------------------------------------------------------+
def ImageMorphologicalOperation(input_image, output_image, operation_labels, operation, kernel, overwrite):
...
def main():
    # Setup description
    description = '''
Function performs morphological operations on labels. These include:
...
labels. The labels are processed in the order that they are defined in the
list. In the cases of dilation, changing the order affects which labels are
written over.
'''

    epilog = '''
Example calls: 
ogoImageMorphologicalOperation /Users/skboyd/Desktop/ML/test/retro_mask.nii.gz /Users/skboyd/Desktop/ML/test/retro_mask_output.nii.gz \
     --operation_labels 1 2 \
     --operation dilation
     
ogoImageMorphologicalOperation image_in.nii.gz image_out.nii.gz --operation closing
...
'''
    ...
    parser = argparse.ArgumentParser(
        ...
        prog="ogoImageMorphologicalOperation",
    ...
    print(echo_arguments('ImageMorphologicalOperation', vars(args)))

    # Run program
    ImageMorphologicalOperation(**vars(args))
